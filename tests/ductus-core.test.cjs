...
assert.match(html, /id="practiceGuidance"/);
assert.match(html, /What to practice/);
...
assert.match(html, /value="sample-i"/);
assert.match(html, /value="sample-u"/);
assert.match(html, /value="sample-e"/);
assert.match(html, /value="sample-r"/);
assert.match(html, /value="join-iu"/);
assert.match(html, /value="sample-m"/);
assert.match(html, /id="adoptionReview"/);
assert.match(html, /id="reviewReferenceHand"/);
assert.match(html, /id="reviewReferenceGroup"/);
assert.match(html, /id="reviewReferenceGlyph"/);
assert.match(html, /id="confirmAdopt"/);
...
assert.equal(JSON.stringify(hands[0].groups.map(group => group.id)), JSON.stringify(['warmups', 'lowercase-stems', 'joins']));
...
assert.equal(JSON.stringify(hands[0].groups[1].referenceIds), JSON.stringify(['sample-i', 'sample-u', 'sample-n', 'sample-m', 'sample-e', 'sample-r']));
assert.equal(JSON.stringify(hands[0].groups[2].referenceIds), JSON.stringify(['join-iu', 'join-nm']));
assert.equal(core.resolvePracticeSelection('kurrent-basic', 'lowercase-stems', '').referenceId, 'sample-i');
const stemRefs = core.referencesForGroup('kurrent-basic', 'lowercase-stems');
assert.equal(JSON.stringify(stemRefs.map(item => item.id)), JSON.stringify(['sample-i', 'sample-u', 'sample-n', 'sample-m', 'sample-e', 'sample-r']));
assert.ok(stemRefs.every(item => core.referenceById(item.id).strokes.length > 0));
assert.ok(stemRefs.every(item => core.referenceById(item.id).strokes.every(stroke => stroke.hint)));
assert.equal(core.referenceById('sample-i').glyph, 'i');
assert.equal(core.referenceById('sample-u').glyph, 'u');
assert.equal(core.referenceById('sample-e').glyph, 'e');
assert.equal(core.referenceById('sample-r').glyph, 'r');
assert.equal(core.referenceById('sample-m').glyph, 'm');
assert.equal(core.referenceById('sample-i').strokes.length, 1);
assert.equal(core.referenceById('sample-u').strokes.length, 2);
assert.equal(core.referenceById('sample-e').strokes.length, 1);
assert.equal(core.referenceById('sample-r').strokes.length, 2);
assert.equal(core.referenceById('sample-m').strokes.length, 3);
...
const defaultGuidance = core.practiceGuidanceFor('kurrent-basic', 'warmups', 'warmup-hairline');
assert.equal(JSON.stringify(core.guidedSessionSequence().map(step => step.referenceId)), JSON.stringify(['warmup-hairline', 'warmup-downstroke', 'warmup-compound', 'sample-i', 'sample-u', 'sample-n', 'sample-m', 'sample-e', 'sample-r', 'join-iu', 'join-nm']));
assert.equal(core.defaultGuidedSession().handId, 'kurrent-basic');
assert.equal(core.defaultGuidedSession().currentStep.referenceId, 'warmup-hairline');
assert.equal(core.defaultGuidedSession().nextStep.referenceId, 'warmup-downstroke');
assert.equal(core.defaultGuidedSession().progressLabel, 'Step 1 of 11');
core.startGuidedSession();
assert.equal(core.guidedSession.active, true);
assert.equal(core.guidedSession.index, 0);
assert.equal(core.reference.glyph, 'hairline');
assert.equal(element('sessionProgress').textContent, 'Step 1 of 11');
assert.ok(element('sessionCurrent').textContent.includes('Hairline'));
assert.ok(element('sessionNext').textContent.includes('Downstroke'));
assert.ok(element('practiceGuidanceTitle').textContent.includes('Hairline'));
core.continueGuidedSession();
assert.equal(core.guidedSession.index, 1);
assert.equal(core.reference.glyph, 'downstroke');
assert.equal(element('sessionProgress').textContent, 'Step 2 of 11');
assert.ok(element('sessionNext').textContent.includes('Compound'));
core.retryGuidedSessionStep();
assert.equal(core.guidedSession.index, 1);
assert.equal(core.reference.glyph, 'downstroke');
assert.equal(core.resolvePracticeSelection('kurrent-basic', 'warmups', 'warmup-downstroke').referenceId, 'warmup-downstroke');
const sessionExport = core.exportAttemptPayload(core.reference, []);
assert.equal(sessionExport.settings.guidedSession.active, true);
assert.equal(sessionExport.settings.guidedSession.currentReferenceId, 'warmup-downstroke');
assert.equal(sessionExport.settings.guidedSession.nextReferenceId, 'warmup-compound');
assert.equal(sessionExport.settings.guidedSession.progressLabel, 'Step 2 of 11');
core.startGuidedSession();
assert.equal(defaultGuidance.title, 'Hairline warm-up');
assert.ok(defaultGuidance.body.includes('light, even upstroke'));
assert.ok(defaultGuidance.cues.some(cue => cue.includes('pressure low')));
assert.equal(element('practiceGuidanceTitle').textContent, 'Hairline warm-up');
assert.ok(element('practiceGuidanceBody').textContent.includes('light, even upstroke'));
assert.ok(element('practiceGuidanceCues').children.some(item => item.textContent.includes('pressure low')));
const downstrokeGuidance = core.practiceGuidanceFor('kurrent-basic', 'warmups', 'warmup-downstroke');
assert.ok(downstrokeGuidance.body.includes('firm'));
assert.ok(downstrokeGuidance.cues.some(cue => cue.includes('release near the baseline')));
const compoundGuidance = core.practiceGuidanceFor('kurrent-basic', 'warmups', 'warmup-compound');
assert.ok(compoundGuidance.body.includes('thin → thick → thin'));
const stemsGuidance = core.practiceGuidanceFor('kurrent-basic', 'lowercase-stems', 'sample-n');
assert.ok(stemsGuidance.body.includes('combines the warm-up movements'));
assert.ok(stemsGuidance.cues.some(cue => cue.includes('stroke order')));
const iGuidance = core.practiceGuidanceFor('kurrent-basic', 'lowercase-stems', 'sample-i');
assert.ok(iGuidance.body.includes('entry stem'));
assert.ok(iGuidance.cues.some(cue => cue.includes('one stroke')));
const uGuidance = core.practiceGuidanceFor('kurrent-basic', 'lowercase-stems', 'sample-u');
assert.ok(uGuidance.body.includes('two connected stems'));
assert.ok(uGuidance.cues.some(cue => cue.includes('second stroke')));
const mGuidance = core.practiceGuidanceFor('kurrent-basic', 'lowercase-stems', 'sample-m');
assert.ok(mGuidance.body.includes('three stem movements'));
assert.ok(mGuidance.cues.some(cue => cue.includes('three-stroke order')));
const eGuidance = core.practiceGuidanceFor('kurrent-basic', 'lowercase-stems', 'sample-e');
assert.ok(eGuidance.body.includes('small loop'));
assert.ok(eGuidance.cues.some(cue => cue.includes('counter')));
const rGuidance = core.practiceGuidanceFor('kurrent-basic', 'lowercase-stems', 'sample-r');
assert.ok(rGuidance.body.includes('shoulder'));
assert.ok(rGuidance.cues.some(cue => cue.includes('second stroke')));
const joinRefs = core.referencesForGroup('kurrent-basic', 'joins');
assert.equal(JSON.stringify(joinRefs.map(item => item.id)), JSON.stringify(['join-iu', 'join-nm']));
assert.ok(joinRefs.every(item => core.referenceById(item.id).strokes.every(stroke => stroke.hint)));
assert.ok(core.practiceGuidanceFor('kurrent-basic', 'joins', 'join-iu').body.includes('join'));

const authoredAttempt = JSON.parse(JSON.stringify(core.referenceById('sample-u').strokes));
const review = core.prepareAdoptionReview({ strokes: authoredAttempt, assignment: { handId: 'kurrent-basic', groupId: 'lowercase-stems', referenceId: 'sample-e' }, metadata: { hand: 'Reviewed Hand', tool: 'review pen', notes: 'Keep this candidate only after checking direction.' } });
assert.equal(review.assignment.handId, 'kurrent-basic');
assert.equal(review.assignment.groupId, 'lowercase-stems');
assert.equal(review.assignment.referenceId, 'sample-e');
assert.equal(review.metadata.hand, 'Reviewed Hand');
assert.equal(review.strokeCount, 2);
assert.equal(review.pointCount, authoredAttempt.flatMap(stroke => stroke.points).length);
assert.ok(review.pressureProfile.variationLabel);
assert.ok(review.summary.includes('2 strokes'));
assert.ok(review.summary.includes('sample-e'));
const reviewedReference = core.referenceFromAdoptionReview(review);
assert.equal(reviewedReference.hand, 'Reviewed Hand');
assert.equal(reviewedReference.tool, 'review pen');
assert.equal(reviewedReference.notes, 'Keep this candidate only after checking direction.');
assert.equal(reviewedReference.glyph, 'e');
assert.equal(reviewedReference.strokes.length, 2);
assert.equal(reviewedReference.strokes[0].t, undefined);
assert.equal(reviewedReference.strokes[0].points[0].t, 0);
assert.equal(reviewedReference.strokes[0].hint, 'Reviewed candidate stroke 1 for Kurrent e · starter.');
assert.deepEqual(core.safeReference(JSON.parse(JSON.stringify(reviewedReference))).glyph, 'e');
...
assert.equal(JSON.stringify(refs.map(r => r.id)), JSON.stringify(['sample-i', 'sample-u', 'sample-n', 'sample-m', 'sample-e', 'sample-r', 'join-iu', 'join-nm', 'warmup-hairline', 'warmup-downstroke', 'warmup-compound']));
