...
assert.equal(JSON.stringify(hands[0].groups[1].referenceIds), JSON.stringify(['sample-i', 'sample-u', 'sample-n', 'sample-m']));
...
assert.equal(JSON.stringify(stemRefs.map(item => item.id)), JSON.stringify(['sample-i', 'sample-u', 'sample-n', 'sample-m']));
...
assert.equal(JSON.stringify(core.guidedSessionSequence().map(step => step.referenceId)), JSON.stringify(['warmup-hairline', 'warmup-downstroke', 'warmup-compound', 'sample-i', 'sample-u', 'sample-n', 'sample-m']));
...
assert.equal(core.defaultGuidedSession().progressLabel, 'Step 1 of 7');
core.startGuidedSession();
assert.equal(core.guidedSession.active, true);
assert.equal(core.guidedSession.index, 0);
assert.equal(core.reference.glyph, 'hairline');
assert.equal(element('sessionProgress').textContent, 'Step 1 of 7');
...
assert.equal(element('sessionProgress').textContent, 'Step 2 of 7');
...
assert.equal(sessionExport.settings.guidedSession.progressLabel, 'Step 2 of 7');
core.startGuidedSession();
...
assert.equal(JSON.stringify(refs.map(r => r.id)), JSON.stringify(['sample-i', 'sample-u', 'sample-n', 'sample-m', 'warmup-hairline', 'warmup-downstroke', 'warmup-compound']));
